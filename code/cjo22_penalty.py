'''
    ...
    初始规则集合
        /data2/zhZH/ljp-llm/mydata/CaseItem/init_penaltyrule_cjo22.json
    
    测试集
        /data2/zhZH/ljp-llm/mydata/pljp/Bert_CJO22test_top10.json
'''
...
config = {
    "readroot" :'/data2/zhZH/ljp-llm/mydata/pljp/Bert_CJO22test_top10.json',
    "smallmodel": "Bert",
    ...
    "penaltyrule":"/data2/zhZH/ljp-llm/mydata/CaseItem/init_penaltyrule_cjo22.json",
    "writeroot": '/data2/zhZH/ljp-llm/log/pljp/llama3/',
    'option_num' : 2,
    'test_len':0.5, # cjo22  0.5=833
    'TASK': { "article": 0, "crime":1, "penalty":2 },
}
acclogpath = f"{config['writeroot']}{config['smallmodel']}/penalty_top5rulestp_CJO_acc.json"
dialogpath = config["writeroot"]+ f'{config["smallmodel"]}/penalty_top5rulestp_CJO_dialog.json'

prompt_tempalte = '''作为中国法律领域的专家，请根据案件事实判断案件是否符合刑期{penalty}的一阶逻辑表达式。
案件细节:
{fact}
一阶逻辑表达式:
{rule}
...
if __name__ == '__main__':
    ...
    lawlablelist = File_op.getlabel_cjo22(config['TASK']['penalty'])
    paRulelist = File_op.getrule_cjo22(config['TASK']['penalty'])
    weijinci = ['毒品','冰毒','海洛因']
    episode_acc = []
