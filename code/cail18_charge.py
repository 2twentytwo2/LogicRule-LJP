...
config = {
    ...
    'option_num' : 2,
    'test_len':0.005, # 0.005=749
    'TASK': { "article": 0, "crime":1, "penalty":2 }
}

prompt_tempalte = '''作为中国法律领域的专家，请根据嫌疑人{criminal}的案件事实判断案件是否符合罪名‘{crime}’的规则。
案件细节:
{fact}
嫌疑人触犯的法律如下:
{articletxts}
...
案件细节: 
{fact}
如果案件细节中有存在{criminal}的犯罪时间、地点、行为、犯罪结果、{criminal}的主观心态等细节请汇总，最后生成少于3000字的摘要。At the end show the abstract bracketed between <abstract> and </abstract>.
'''

acclogpath = f"{config['writeroot']}/charge_top5Upadaterulestp_Cailcharges_acc.json" # 修改了charges
dialogpath = config["writeroot"]+ '/charge_top5Upadaterulestp_Cailcharges_dialog.json'
...
def checkbyAllChargeRule(jsoncase):
    for onetop in crimelablelist:
        ...
        rule = ruledict[onetop]
        articletxt = getArtcletxt(onetop)
        true_article = jsoncase['article']
        true_articletxt = alllawtxt[true_article]
        crime = onetop
        # fact过长
        if len(fact) > 5000:
        ...
        simcase1_charge = crimelablelist[0]
        mesage = prompt_tempalte.format(criminal=criminal,fact=fact,articletxts=true_articletxt,rule=rule,crime=crime,simcase1_article=simcase1_charge,simcase2_article=charge)
...
if __name__ == '__main__':
    ...
    alllawtxt = File_op.readalllawtext()

    testjsoncase = File_op.readjsonlist(readpath)
    # crimelablelist, crimelabel2Intdict = File_op.readcrimelabellist()
    crimelablelist = File_op.getlabel_cail2018(config['TASK']['crime'])
    crimelabel2Intdict = File_op.getCrimelabelDict(crimelablelist)
    ruledict = File_op.readCrimeRule() # key:crime, item:rulestr
    ...
    for train_e in range(4):
        ...
        for jsoncase in testcase:
            ...
            index += 1
            episode_charge = []
            charges = [jsoncase['top5_charge'][0]]
            ...
            for i in range(10): # 使用10轮，llama3会出现回答不一致的情况，多次询问选择出现次数最多的答案
                # charge 
                answer = checkbyChargeRule(jsoncase,charges) # jsoncase["top5_charge"]
